# System prompt — verbatim source

This is the verbatim text the user supplied for the chat (assessment) model's system prompt, with the §11 spec addition inserted in the Scoring System subsection directly after the confidence-check paragraph (marked **[§11 ADDITION]** below).

When implementing Task 6 of the plan, paste this entire prompt (without the heading and the "[§11 ADDITION]" markers) into the `SYSTEM_PROMPT` template literal in `lib/prompts/system.ts`.
...
**[§11 ADDITION — inserted here per spec §11]**
